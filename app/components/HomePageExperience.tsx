...
import { createPageSchema, reviewSchemas } from '../data/seo';
import { services } from '../data/services';

export const homeTitle = 'Digital Agency in India | Kraftt Digital';
export const homeDescription = 'Kraftt Digital connects brand, websites, content and digital systems for businesses in India—making them easier to discover, trust and choose.';
...
function balancedTitleLines(title: string): [string, string] {
  ...
  for (let index = 0; index < words.length - 1; index += 1) {
    ...
    splitAt = index + 1;
    if (currentLength >= midpoint) break;
  }
...
export function HomePageExperience({ eyebrow, intro, schema, geographicContext }: HomePageExperienceProps = {}) {
  ...
  const geographicTitleLines = geographicContext ? balancedTitleLines(geographicContext.localTitle) : null;
    ...
    <main>
      <JsonLd data={{
        ...createPageSchema({
          ...
          entities: [...reviewSchemas(homeReviews), ...(schemaContext.entities ?? [])],
        }),
      }} />
      <SiteHeader overlay />
      ...
      <section className="home-case-playground section-light">
        <Reveal className="home-case-playground-heading">
          ...
          <div className="home-case-playground-intro">
            <p>{geographicContext?.proofNote ?? 'Four connected digital systems, each shaped around a different business gap. Every outcome is labelled by the evidence available.'}</p>
                ...
                <Reveal className={`home-service-play-card home-service-play-card-${index + 1}`} delay={index * 0.06} key={category.title}>
                  ...
                  <div className="home-service-play-links">
                    {categoryServices.map((service, serviceIndex) => (
                      <Link key={service.slug} href={geographicContext?.enabledServiceSlugs.includes(service.slug) ? `${geographicContext.serviceBasePath}/${service.slug}` : `/services/${service.slug}`}>
      ...
      <section className="home-trust">
        <div className="home-trust-inner">
          <Reveal className="home-trust-heading" direction="scale">
            ...
            <h2>{geographicContext ? <>Questions, answered.<br /><em>Before you enquire.</em></> : <>Clear work.<br /><em>Clear value.</em></>}</h2>
            <div className="home-trust-heading-intro">
              <p>{geographicContext ? `Clear, factual answers about working with Kraftt for a business in ${geographicContext.marketName}.` : 'Kraftt makes the thinking, scope, pricing and ownership clear before the project begins—so trust does not depend on a sales promise.'}</p>
              <Link href={geographicContext ? '/contact' : '/process'}>{geographicContext ? 'Ask another question' : 'See how we work'} <span aria-hidden="true">↗</span></Link>
            </div>
          </Reveal>
          ...
          <div className="home-trust-story">
            ...
            <div className="home-trust-commitments" aria-label={geographicContext ? `Frequently asked questions about working with Kraftt in ${geographicContext.marketName}` : 'Kraftt commitments'}>
