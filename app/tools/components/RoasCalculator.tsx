...
export function RoasCalculator() {
  ...
  return <>
    <form className="tool-calculator tool-wrap-wide" id="tool-workspace" onSubmit={calculate}>
      ...
      <div className="tool-field-grid"><Field label="Ad spend (₹)"><input type="number" min="0.01" step="0.01" value={spend} onChange={(event) => { setSpend(event.target.value); setShowResult(false); }} placeholder="25000" required /></Field><Field label="Revenue from those ads (₹)"><input type="number" min="0" step="0.01" value={revenue} onChange={(event) => { setRevenue(event.target.value); setShowResult(false); }} placeholder="100000" required /></Field><Field label="Profit margin (%) — optional" hint="Add this to see break-even ROAS and profit after ads."><input type="number" min="0.01" max="100" step="0.1" value={margin} onChange={(event) => { setMargin(event.target.value); setShowResult(false); }} placeholder="40" /></Field></div>
